<template>
  <div
    id="googleSignIn"
    :class="typeof className !== 'undefined' ? className : null"
  >
    ...
    gapi.load("auth2", function () {
      ...
      try {
        if (window.__google_oauth_useful_variables.customButtonId) {
          window.__google_oauth_useful_variables.attachSignin(
            document.getElementById(
              window.__google_oauth_useful_variables.customButtonId
            )
          );
        }
      } catch (e) {
  ...
  methods: {
    validate: function () {
      if (typeof this.successCallBack === "undefined")
        this.warning
          ? console.warn(
              "Google Sign In --> Specify the success callback method to get the data"
            )
          : null;
      else if (typeof this.successCallBack !== "function")
        this.warning
          ? console.warn(
              "Google Sign In --> Success Callback must be a valid Function"
            )
          : null;
      if (typeof this.failureCallBack === "undefined")
        this.warning
          ? console.warn(
              "Google Sign In --> Specify the Failure callback method to get the Error data"
            )
          : null;
      else if (typeof this.failureCallBack !== "function")
        this.warning
          ? console.warn(
              "Google Sign In --> Failure Callback must be a valid Function"
            )
          : null;
      if (this.customButton) {
        if (typeof this.customButtonId === "undefined")
          this.warning
            ? console.warn(
                "Google Sign In --> Please Provide customButtonId or change the customButton to false"
              )
            : null;
      }
    },
